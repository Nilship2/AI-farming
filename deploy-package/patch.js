...
window.showDetail = function(type, id) {
    ...
    if (type === "crop") {
        ...
        h += '<div>\u{1f4e6} \u6536\u83b7\u4ea7\u51fa: <strong>'+getHarvestAmt(cd)+'\u4e2a\u4f5c\u7269</strong></div>';
        ...
        var _poolInfo = formatPoolInfo(cd);
        if (_poolInfo) h += '<div style="margin-top:6px;border-top:1px solid #5a3a1a;padding-top:6px">📊 概率掉落:</div>' + _poolInfo;
        h += '</div>';
    } else if (type === "animal") {
    ...
    for (var si = 0; si < invSpans.length; si++) {
        (function(span) {
            ...
            var items = [
                ...
                {k:'tomato',icon:'🍅',n:'番茄',v:35},{k:'pepper',icon:'🌶️',n:'辣椒',v:40},{k:'blueberry',icon:'🫐',n:'蓝莓',v:60},{k:'watermelon',icon:'🍉',n:'西瓜',v:160},{k:'grape',icon:'🍇',n:'葡萄',v:130},{k:'sunflower',icon:'🌻',n:'向日葵',v:45},{k:'carrot_pumpkin',icon:'✨🥕',n:'金色胡萝卜',v:120},{k:'corn_blueberry',icon:'💜🌽',n:'紫玉米',v:100},{k:'tomato_pepper',icon:'🔥🍅',n:'火焰番茄',v:90},{k:'wheat_sunflower',icon:'🌟🌾',n:'金穗麦',v:150},{k:'strawberry_grape',icon:'💎🍓',n:'宝石莓',v:200},{k:'watermelon_pumpkin',icon:'🏆🎃',n:'巨无霸瓜',v:300},{k:'blueberry',icon:'🫐',n:'蓝莓',v:60},{k:'watermelon',icon:'🍉',n:'西瓜',v:160},{k:'grape',icon:'🍇',n:'葡萄',v:130},{k:'sunflower',icon:'🌻',n:'向日葵',v:45},
...
// ===================================================================
(function() {
    var _renderSystem = renderSystem;
    renderSystem = function() {
        _renderSystem();
        var panel = document.getElementById("psys");
        if (!panel || (" " + panel.className + " ").indexOf(" ac ") === -1) return;

        var h = '<div class="cd"><h3>💾 存档位管理</h3>';
        h += '<div style="font-size:.8em;color:#aaa;margin-bottom:6px">当前槽位: <span style="color:#ffd700">' + (window.__saveSlot || 0) + '</span> | 切换存档需刷新页面</div>';

        for (var i = 0; i < 5; i++) {
            var info = (typeof getSaveSlotInfo === "function") ? getSaveSlotInfo(i) : null;
            var isCurrent = (window.__saveSlot || 0) === i;
            h += '<div class="mr" style="' + (isCurrent ? 'border:1px solid #ffd700;' : '') + '">';
            h += '<div style="flex:1">';
            h += '<span style="font-weight:bold;color:' + (isCurrent ? '#ffd700' : '#ffcc80') + '">槽位 ' + i + (isCurrent ? ' ◀ 当前' : '') + '</span>';
            if (info) {
                var seasonNames = ["春","夏","秋","冬"];
                h += '<div class="tt">💰 ' + info.coins + ' | 第' + info.year + '年 ' + seasonNames[info.season || 0] + ' | ⭐' + info.prestige + '</div>';
                h += '<div class="tt">Mod: ' + (info.modCount > 0 ? info.mods.join(', ') : '无') + '</div>';
            } else {
                h += '<div class="tt" style="color:#888">空</div>';
            }
            h += '</div>';
            h += '<div style="display:flex;gap:4px">';
            if (!isCurrent && info) {
                h += '<button class="bt sm bl" onclick="if(typeof switchSaveSlot===\'function\')switchSaveSlot(' + i + ')">切换</button>';
            }
            if (info) {
                h += '<button class="bt sm rd" onclick="if(confirm(\'删除槽位 ' + i + ' 的存档？不可恢复！\')){if(typeof deleteSaveSlot===\'function\')deleteSaveSlot(' + i + ');location.reload();}">删除</button>';
            }
            h += '</div>';
            h += '</div>';
        }
