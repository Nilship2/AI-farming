...
(function(){
    ...
    loadGame = function(){
        ...
        if(typeof d.coins!=="number"||typeof d.totalCoinsEarned!=="number"){initGame();return false;}
        if(!d.regions&&d.land){
...
initGame = function(){
    ...
    if(!GS.extraLand)GS.extraLand=0;GS.scarecrowOn=true;GS.greenhouseOn=true;if(GS.upgrades.drone&&GS.droneOn===undefined)GS.droneOn=true;
...
// ============================================================
// Override renderBestiary — use formatTime and getHarvestCount
// ============================================================
(function(){
    ...
    renderBestiary = function(){
        ...
        if(!pb)return;
        // Post-process: fix growth time format and harvest count display
        var sls=pb.querySelectorAll(".sl");
        var cropKeys=Object.keys(CROP_DEFS).filter(function(k){return !CROP_DEFS[k].isHybrid;});
        var ci=0;
        for(var i=0;i<sls.length;i++){
            var sl=sls[i];
            if(sl.getAttribute("data-action"))continue;
            if(ci<cropKeys.length){
                var cd=CROP_DEFS[cropKeys[ci]];
                var hc=getHarvestCount(cd);
                var tts=sl.querySelectorAll(".tt");
                for(var j=0;j<tts.length;j++){
                    var tt=tts[j];
                    tt.textContent=tt.textContent.replace(/生长:\d+s/,"生长:"+formatTime(cd.g||0));
                    tt.textContent=tt.textContent.replace(/产出:\d+个/,"产出:"+hc+"个");
                }
                ci++;
            }
        }
