...
function showPlants(sid){GS._planting=sid;var av=[];for(var k in CROP_DEFS){if(GS.discoveredCrops.indexOf(k)!==-1||CROP_DEFS[k].unlock<=GS.totalCoinsEarned)av.push(k);}var h='<div class="cd"><h3>选择种植</h3><div class="cg" style="grid-template-columns:repeat(5,1fr)">';for(var i=0;i<av.length;i++){var cid=av[i];var cd=CROP_DEFS[cid];var sf=0;if(GS.relics)for(var ri4=0;ri4<GS.relics.length;ri4++){var rd4=RELIC_DEFS[GS.relics[ri4]];if(rd4&&rd4.ef==="seedDiscount")sf+=rd4.v;}var sc=Math.ceil(cd.v*0.25*(1-Math.min(0.9,sf)));h+='<div class="sl" data-action="doPlant" data-sid="'+sid+'" data-cid="'+cid+'"><div style="font-size:2em">'+(cd.i||'🌱')+'</div><div>'+cd.n+'</div><div class="tt">生长:'+cd.g+'s | 产出:'+getHarvestAmt()+'个</div><div class="tt">需要种子:'+sc+'</div></div>';}h+='<button class="bt sm rd" data-action="cancelPlant" style="margin-top:8px">✕ 取消</button></div></div>';var pf=document.getElementById("pf");if(pf)pf.innerHTML=h;}
...
function getHarvestAmt(){var a=2;if(GS.gemUpgrades&&GS.gemUpgrades.harvestBless)a++;if(GS.gemUpgrades&&GS.gemUpgrades.harvestBless2)a++;return a;}function renderBestiary(){
...
h+='<div class="sl'+(dv?'':' lk')+'"><div style="font-size:2em">'+(dv?d.i:'❓')+'</div><div>'+(dv?d.n:'???')+'</div>'+(dv?'<div class="tt">生长:'+d.g+'s | 产出:'+getHarvestAmt()+'个</div>':'')+'</div>';}
h+='</div></div>';
