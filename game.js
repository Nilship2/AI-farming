...
function renderPrestige(){var pts=Math.floor(Math.sqrt(Math.max(0,GS.totalCoinsEarned)/10000));var h='<div class="cd"><h3>🔄 转生系统</h3>';h+='<p>累计获得 <strong>'+Math.floor(GS.totalCoinsEarned).toLocaleString()+'</strong> 金币</p>';h+='<p>转生后可获得 <strong>'+pts+'</strong> 个农场之星</p>';h+='<p>当前农场之星：<strong>'+GS.prestigePoints+'</strong>（金币收入 +'+GS.prestigePoints*10+'%）</p>';h+='<p class="tt">转生将重置所有进度，但保留：图鉴、杂交品种、成就、故事碎片</p>';h+='<p class="tt">转生后解锁更多土地和畜棚容量</p>';h+='<button class="bt" data-action="doPrestige"'+(pts<1?' disabled':'')+'>🔄 转生获得 '+pts+' 个农场之星</button>';h+='</div>';var ppr=document.getElementById("ppr");if(ppr)ppr.innerHTML=h;}
