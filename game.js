...
function buyGemUpgrade(gid){var gd=GEM_UPG_DEFS[gid];if(!gd)return;if(GS.gemUpgrades[gid])return;if(gd.req&&!GS.gemUpgrades[gd.req]){notify('需要先购买'+GEM_UPG_DEFS[gd.req].n);return;}var price=gd.c;if(gd.repeatable)price=gd.c+(GS.extraLand||0)*gd.costGrowth;if(GS.gems<price){notify('宝石不足！');return;}GS.gems-=price;GS.gemUpgrades[gid]=true;if(gd.repeatable){GS.gemUpgrades[gid]=false;GS.extraLand=(GS.extraLand||0)+1;}notify('💎 已购买: '+gd.n);renderPrestige();R();saveGame();}function sellItem(k,q){
...
function gameLoop(){
var now=Date.now();var dt=(now-lastTick)/1000;lastTick=now;if(dt>5)dt=5;if(dt>5)dt=5;
tick(dt);
_renderInterval+=dt;
